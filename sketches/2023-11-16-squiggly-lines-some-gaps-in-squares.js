...
const makeLines = ([fromX, fromY], width, height) => {
  ...
  for (let i = 0; i <= HOW_MANY_LINES; i++) {
    ...
    while (y <= fromY + height) {
      ...
      const end = newY + GAP_SIZE >= fromY + height ? fromY + height : newY;
      const segments = Math.round(
        (end - y) / 0.1 + (Math.random() * 0.04 - 0.02)
      );

      Array.from({ length: segments }).forEach((v, i) => {
        lines.push([
          [x + (i % 2 === 1 ? 0.01 : -0.01), y + ((end - y) / segments) * i],
          [
            x + (i % 2 === 0 ? 0.01 : -0.01),
            y + ((end - y) / segments) * (i + 1),
          ],
        ]);
      });
